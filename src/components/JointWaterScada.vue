...
const SILVER = { type: 'linearGradient', stops: [
...
const STEEL = { type: 'linearGradient', stops: [
  { offset: '0%', color: '#d4dbe2' }, { offset: '50%', color: '#9aa3ad' }, { offset: '100%', color: '#727b85' }], attrs: { x1: '0%', y1: '0%', x2: '100%', y2: '0%' } }

// ---- ports ----
const portGrp = (attrs) => ({ position: { name: 'absolute' }, attrs: { circle: { r: 7, fill: '#cfd6de', stroke: '#8a929c', strokeWidth: 1.5, magnet: 'passive', ...attrs } }, markup: svg`<circle @selector="circle"/>` })
const portsCfg = (items) => ({ groups: { p: portGrp() }, items: items.map(i => ({ group: 'p', id: i.id, args: { x: i.x, y: i.y } })) })

// ---- shapes ----
// Tank-1 level scale 0..100 every 10 (static markup nodes)
const WIN_Y = 36, WIN_H = 178 // window spans y 36..214 (height 250)
const scaleNodes = []
...
const CylTank = joint.dia.Element.define('s.Cyl', { size: { width: 150, height: 250 }, attrs: {
...
const Hopper = joint.dia.Element.define('s.Hopper', { size: { width: 170, height: 230 }, attrs: {
...
const Pump = joint.dia.Element.define('s.Pump', { size: { width: 92, height: 92 }, attrs: {
...
const Valve = joint.dia.Element.define('s.Valve', { size: { width: 76, height: 92 }, attrs: {
...
const Zone = joint.dia.Element.define('s.Zone', { size: { width: 110, height: 40 }, attrs: {
...
const PGauge = joint.dia.Element.define('s.PG', { size: { width: 96, height: 96 }, attrs: {
